...
#    August 2012. Modified the script to work with Sportstracker data.
#    Sportstracker uses a non standard date format and if you have a
#    heart rate monitor, the data is exported as part of the gpx file.
#    Rather than introduce a whole range of extra command line switches
#    I decided to make this into a separate script.

#    The script works fine if you don't have a heart monitor, but all
#    heart rate values will be recorded as zero.
...
# Sportstracker uses a non stabdard date format.
    ...
    for seg in trksegs:
        ...
        for pt in trkpts:
            ...
            def prettify_time(time):
                ...
                #print "Date in sportstrackerformat"
                time=sub(r'\.\d+$','',time)
            ...
            if ele:
                ...
                #if sportstracker:
                #s.append([lat, lon, time, ele, hrData])
                #else:
                #print hrData
            s.append([lat, lon, time, int(hrData.text), ele])
        ...
        if len(seg)>0:
            ...
            for pt in seg:
                ...
                if prev_lat and prev_lon:
                    ...
                    vel=0.0
                dist=dist+delta
                newseg.append([lat,lon,time,ele,dist,vel,hr])
...
def print_gpx_trk(trk,file=sys.stdout,metric=True):
    f=file
    if metric:
        f.write('# time(ISO) elevation(m) distance(km) velocity(km/h) heart rate(bpm)\n')
        km,m=1.0,1.0
    else:
    ...
    for seg in trk:
        ...
        for p in seg:
            f.write('%s %f %f %f %d\n'%\
                ...
                m*p[var_ele],km*p[var_dist],km*p[var_vel],p[var_hr])))
        f.write('\n')
...
def main():
	...
	npoints=None
        ...
        parser.add_argument("--y-axis", dest="yvar", default="elevation", choices=["elevation", "velocity", "heartrate"], help="plot  elevation (default), velocity or heart rate on y-axis")
